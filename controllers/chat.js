...
const newGroupChat = TryCatch(async (req, res, next) => {
  ...
  //this spread member and the user
...
const addMembers = TryCatch(async (req, res, next) => {
  ...
  //forbid to add member if not creator
...
const removeMember = TryCatch(async (req, res, next) => {
  ...
  const [chat, usersThatWillBeRemoved] = await Promise.all([Chat.findById(chatId), User.findById(userId, "name")]);
  ...
  if (chat.members.length <= 3) return next(new ErrorHandler("Group must have at lease 3 members", 400));
  ...
  emitEvent(req, ALERT, chat.members, { message: `${usersThatWillBeRemoved.name} has been removed from the group`, chatId });
...
const leaveGroup = TryCatch(async (req, res, next) => {
  ...
  //remaining members excluding user
  ...
  //query user model to find user by their id and selecting only the name field
...
const sendAttachMents = TryCatch(async (req, res, next) => {
  ...
  //if file exists is truthu not null otherwise it assign an empty array to files
  const files = req.files || [];

  if (files.length < 1) return next(new ErrorHandler("Please provide attachments", 400));
  if (files.length > 5) return next(new ErrorHandler("Cannot add more than 5 files", 400));

  const [chat, me] = await Promise.all([Chat.findById(chatId), User.findById(req.user, "name")]);
  if (!chat) return next(new ErrorHandler("Chat not found", 404));

  //sending attachments here
...
const deleteChat = TryCatch(async (req, res, next) => {
  ...
  //delete the chat including attachments from cloudinary..

  //mongo db query condition to check whether att exist or not
...
const getMessages = TryCatch(async (req, res, next) => {
  ...
  //zero skip means updated message
  const skip = (page - 1) * resultPerPage;

  const chat = await Chat.findById(chatId);
  if (!chat) return next(new ErrorHandler("Chat not Found", 404));
  //to not let the removed user access the chat
